...
if os.path.exists(os.path.join(PGC_HOME, "python37")):
  sys.path.append(os.path.join(PGC_HOME, "hub", "sripts"))
  sys.path.append(os.path.join(PGC_HOME, "hub", "sripts", "lib"))
...
if "--debug" in args:
  ...
  logging.getLogger('pgcli_logger').setLevel(logging.DEBUG)
  my_logger.debug("DEBUG enabled")

if "--debug2" in args:
  args.remove('--debug2')
  my_logger.info("Enabling DEBUG2 mode")
  logging.getLogger('pgcli_logger').setLevel(pgclog.DEBUG2)
  my_logger.debug("DEBUG enabled")
...
if (p_mode in no_log_commands) and (isJSON == True):
  ...
  my_logger.command("pgc %s", full_cmd_line)
...
try:
  ...
  if p_mode == "discover":
    ...
    if len(args) == 2:
      ...
      util.exit_message("try:  pgc discover [ " + str(vList) + " ]", 1, isJSON)
